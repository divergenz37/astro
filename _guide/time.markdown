...
UT1 is defined by a linear relationship with the Earth Rotation Angle[^iers-tn], and is equivalent to UT0 corrected for the effect of Earth's polar motion on the observer's longitude [^nist-u-w]. Being derived from the ERA and thererfore from Very Long Baseline Interferometry of distant quasars, the position of the Sun is not directly involved in determining UT1.
...
Because TT has a constant time step, unlike the non-uniform rotation used in UT, the difference TT - UT1 varies over time and is known as ΔT[^hmnao-delta-t].
...
Mean solar time is based on the hour angle of the fictitious mean Sun, while apparent solar time is based on the hour angle of the true Sun. The difference between the apparent solar time and the mean solar time is the *equation of time*. Although sign conventions vary, here we take a positive value to mean that apparent solar time is faster than mean solar time. 

## Analemma

The combined effects of the Sun's changing declination and the equation of time means that the Sun's position in the sky at mean noon (noon in mean solar time) varies over the course of a year, and the path traced out is known as an *analemma*.

The figure below shows the approximate shape of the analemma, by plotting the true Sun's declination against equation of time, represented here as the difference in hour angles of the true Sun and mean Sun. 

![Analemma](./figures/analemma.png)
...
As a historical note, the *Besselian year* is defined to begin when the mean longitude of the Sun (affected by aberration) is 280° as measured from the mean equinox of the date[^meeus-1998].
...
[^hmnao-delta-t]: HM Nautical Almanac Office: Delta T [Internet]. [cited 2020 Feb 6]. Available from: <http://astro.ukho.gov.uk/nao/miscellanea/DeltaT/>
[^iau-nfa-glossary]: IAU 2006 NFA Glossary [Internet]. [cited 2020 Feb 5]. Available from: <https://syrte.obspm.fr/iauWGnfa/NFA_Glossary.html>
[^iers]: IERS - IERS - ICRF [Internet]. [cited 2020 Jan 23]. Available from: <https://www.iers.org/IERS/EN/DataProducts/ICRF/icrf.html>
[^iers-glossary-ut1]: IERS - Universal Time [Internet]. [cited 2020 Feb 6]. Available from: <https://www.iers.org/IERS/EN/Service/Glossary/ut1.html>
